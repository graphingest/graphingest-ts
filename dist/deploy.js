...
function findSourceFiles(dir, ext) {
    ...
    function walk(current) {
        for (const entry of (0, node_fs_1.readdirSync)(current, { withFileTypes: true })) {
            ...
            else if ((0, node_path_1.extname)(entry.name) === ext) {
                try {
                    const content = (0, node_fs_1.readFileSync)(full, "utf-8");
...
async function deploy(options) {
    ...
    if (sourceFiles.length === 0) {
        throw new Error(`No TypeScript/JavaScript files with node() or graph() found in ${projectDir}`);
    }
    console.log(`  Found ${sourceFiles.length} file(s) with node()/graph() calls`);
    // 2. Read env file (only if envPath provided)
    let envVars = {};
    if (options?.envPath) {
        const resolved = (0, node_path_1.resolve)(projectDir, options.envPath);
        ...
        if (Object.keys(envVars).length > 0) {
            ...
            console.log(`  Warning: ${options.envPath} not found or empty — using dashboard variables only`);
        }
    }
    else {
    ...
    const resolvedPkgJson = (0, node_path_1.resolve)(projectDir, pkgJsonPath);
    let dependencies;
    if ((0, node_fs_1.existsSync)(resolvedPkgJson)) {
        try {
            const pkg = JSON.parse((0, node_fs_1.readFileSync)(resolvedPkgJson, "utf-8"));
        ...
        console.log(`  No ${pkgJsonPath} found — only graphingest will be installed`);
    }
    // 4. Prepare payload
    const files = {};
    for (const filepath of sourceFiles) {
        const relPath = (0, node_path_1.relative)(projectDir, filepath);
        files[relPath] = (0, node_fs_1.readFileSync)(filepath, "utf-8");
    }
    const payload = {
        files,
        dependencies,
    ...
    const client = new client_1.GraphIngestClient();
    const res = await fetch(`${client.baseUrl}/api/deploy`, {
        method: "POST",
        headers: {
